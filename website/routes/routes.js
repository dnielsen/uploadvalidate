  ...
  http.get(endpointURL.concat('/uploadedImages'), (response) => {
    ...
    response.on('end', () => {
      try {
        const responseArray = [];
        responseData = JSON.parse(responseData);
        if (responseData.srNo) {
          responseData.srNo.forEach((sr, i) => {
            responseArray.push({
              index: sr+1,
              path: responseData.paths[i],
              name: responseData.imageNames[i],
              category: responseData.categories[i]
            });
          });
        }
        res.render('uploadedImages', { title: responseData.title, responseArray, listExists: responseArray.length > 0 });
      }
      catch (error) {
          console.error('Got error in parsing data:',error.message);
          throw error;
  ...
  http.get(endpointURL.concat('/validatedImages'), (response) => {
    ...
    response.on('end', () => {
      try {
        ...
        res.render('validatedImages', { title: responseData.title, responseArray, listExists: responseArray.length > 0 });
      }
      catch (error) {
          console.error('Got error in parsing data:',error.message);
          throw error;
